...
echo "$borrow"
echo ""
echo "----- Borrowing book with isbn2  kodjo@email.com ---"
...
echo "$borrow"
echo ""
echo "----- Borrowing another book with isbn3  kodjo@email.com ---"
...
echo "$borrow"
echo ""
echo "----- Borrowing another book with isbn4 for userowner@email.com  ---"
borrow=$(curl -s -X POST http://localhost:8080/borrowBook -H "Content-Type: application/json" -d '{"email": "kodjo@email.com", "isbn": "isbn3"}')
echo "$borrow"
echo ""
echo ""
